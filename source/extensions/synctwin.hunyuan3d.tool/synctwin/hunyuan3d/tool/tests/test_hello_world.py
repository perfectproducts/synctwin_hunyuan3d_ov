...
import synctwin.hunyuan3d.tool
...
class Test(omni.kit.test.AsyncTestCase):
    ...
    async def test_hello_public_function(self):
        result = synctwin.hunyuan3d.tool.some_public_function(4)
        self.assertEqual(result, 256)

    async def test_window_button(self):
        # Find a label in our window
        label = ui_test.find("SyncTwin Hunyuan3D Tool//Frame/**/Label[*]")

        # Find buttons in our window
        add_button = ui_test.find(
            "SyncTwin Hunyuan3D Tool//Frame/**/Button[*].text=='Add'"
        )
        reset_button = ui_test.find(
            "SyncTwin Hunyuan3D Tool//Frame/**/Button[*].text=='Reset'"
        )
